...
file_train = f"/home/uni08/osatohanmwen/2NP_final/Dataset_Parent1_CVOO/xtrain_{i}.csv"
file_test  = f"/home/uni08/osatohanmwen/2NP_final/Dataset_Parent1_CVOO/xtest_{i}.csv"
...
output_dir = "/home/uni08/osatohanmwen/2NP_final/Result_Parent1_CVOO"
...
data_accuracy = pd.DataFrame(
    ...
    'Fold'     : f'Parent1_CVOO_{i}'
    }, index=[0]
)


data_pre_observed.to_csv(os.path.join(output_dir, f"{trait}_LGBM_OP_Parent1_CVOO_{i}.csv"), index=False)
data_accuracy.to_csv(os.path.join(output_dir, f"{trait}_LGBM_ACC_Parent1_CVOO_{i}.csv"), index=False)

###feature_importance
FI_Lgbm= feature_importances(LGBr_clf_best,column_names)
FI_Lgbm.to_csv(os.path.join(output_dir, f"{trait}_FI_Lgbm_{i}_Parent1_CVOO.csv"), index=False)
