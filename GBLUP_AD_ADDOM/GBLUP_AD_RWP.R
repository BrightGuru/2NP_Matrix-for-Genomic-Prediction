...
for (time in 1:1){
  ...
  cross_predict_add <- foreach(
    ...
    yobs <- y[Test]
    yhat <- as.matrix(funout$uhat[Test]) + (X_test %*% beta)
    
    # Save predicted vs observed
    data_pre_observed <- data.frame(
      Observed = yobs,
      Predicted = yhat
    )
    
    file_name <- paste0(trait, '_Add_RollingPred_', test_year, '_Rep_', time, '.csv')
...
write.table(RESULT_add,file = paste0(trait,"_Add_Rolling_Result", ".csv"),col.names = T,row.names = F,sep = '\t',quote = F)
