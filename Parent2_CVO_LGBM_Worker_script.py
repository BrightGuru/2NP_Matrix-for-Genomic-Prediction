...
file_train = f"/home/uni08/osatohanmwen/2NP_final/Dataset_Parent2_CVO/xtrain_{i}.csv"
file_test  = f"/home/uni08/osatohanmwen/2NP_final/Dataset_Parent2_CVO/xtest_{i}.csv"
...
output_dir = "/home/uni08/osatohanmwen/2NP_final/Result_Parent2_CVO"
...
data_accuracy = pd.DataFrame(
    {'r2_score' : metrics.r2_score(ytest, y_pred_grid),
      ...
      'Fold'     : f'Parent2_CVO_{i}'
    }, index=[0]
)


data_pre_observed.to_csv(os.path.join(output_dir, f"{trait}_LGBM_OP_Parent2_CVO_{i}.csv"), index=False)
data_accuracy.to_csv(os.path.join(output_dir, f"{trait}_LGBM_ACC_Parent2_CVO_{i}.csv"), index=False)

###feature_importance
FI_Lgbm= feature_importances(LGBr_clf_best,column_names)
FI_Lgbm.to_csv(os.path.join(output_dir, f"{trait}_FI_Lgbm_{i}_Parent2_CVO.csv"), index=False)
...
#forest_importances.to_csv(os.path.join(output_dir, f"FI_perm_{i}_CVO.csv"), index=False)


print(f'Parent2_CVO_{i}')
