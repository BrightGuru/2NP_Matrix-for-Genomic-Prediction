...
output_dir = output_dir = "/home/uni08/osatohanmwen/2NP_final/Dataset_Parent1_CVO"
...
for repeat in range (n_repeats):
    ...
    for test_index, _ in gkf.split(Test_set, groups=Test_set['Hybrid']):
        ...
        xtrain = Train_set[
        Train_set['Parent_1'].isin(xtest['Parent_1'])]
        ...
        # Calculate Desired Training Set Size
        desired_train_size = len(set(xtest['Hybrid'])) / 0.2
        current_train_size = len(set(xtrain['Hybrid']))

        # Determine Additional Hybrids Needed
        additional_needed = max(0, int(desired_train_size - current_train_size))
        unused_hybrids = list(set(Train_set['Hybrid']) - set(xtest['Hybrid']))
        
        # Add Exactly the Needed Hybrids
        # Ensure there are available hybrids to add
        additional_hybrids = []
        if additional_needed > 0 and len(unused_hybrids) > 0:
            random.seed(repeat)  # Ensure different seed for each repeat
            additional_hybrids = random.sample(unused_hybrids, k=min(additional_needed, len(unused_hybrids)))

        # Expand xtrain with additional hybrids
        xtrain = Train_set[Train_set['Hybrid'].isin(additional_hybrids + xtest['Hybrid'].tolist())].reset_index(drop=True)

        # Test-to-Training Hybrid Ratio (After Adjustment)
        Test_train_ratio_after = len(set(xtest['Hybrid'])) /len(set(xtrain['Hybrid']))
        print(f"Test to train ratio (after adjustment): {Test_train_ratio_after:.3f}")
