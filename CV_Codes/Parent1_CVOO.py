...
output_dir = output_dir = "/home/uni08/osatohanmwen/2NP_final/Dataset_Parent1_CVOO"
...
for repeat in range(n_repeats):
    ...
    for test_index, _ in gkf.split(Test_set, groups=Test_set['Hybrid']):
       ...
       # Ensure NO Parent_1 Hybrid in Train set appears in Test set
        xtrain = Train_set[~Train_set['Parent_1'].isin(xtest['Parent_1'])]

        # Check if test Parent_1 hybrids were successfully excluded from train set
        assert set(xtest['Parent_1']) & set(xtrain['Parent_1']) == set(), "🚨 Overlap detected between training and test Parent_1 hybrids!"

        # Print details
        # Print details
        #print(f"✅ Repeat {repeat}, Fold {cnt}: xtest shape: {xtest.shape}, xtrain shape: {xtrain.shape}")
